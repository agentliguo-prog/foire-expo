...
export const StandsSection: React.FC<StandsSectionProps> = ({ onSelectStand }) => {
  return (
    <section id="stands" className="py-20 sm:py-28 px-4 sm:px-6 lg:px-8 max-w-[1100px] mx-auto relative z-10">
      {/* Halo lumineux d'arrière-plan */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] bg-brand-blue/10 blur-[130px] rounded-full pointer-events-none -z-10"></div>

      {/* En-tête de section */}
      <div className="text-center max-w-3xl mx-auto mb-16">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-light/10 border border-brand-light/30 text-brand-light text-xs font-semibold uppercase tracking-wider mb-4 shadow-blue-glow">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
        {STANDS_DATA.map((stand) => {
          ...
          return (
            <div
              key={stand.id}
              className={`relative flex flex-col justify-between rounded-2xl p-6 sm:p-8 transition-all duration-300 hover-lift ${
                isPopular
            ...
            >
              {/* Badge Recommandé */}
              {stand.badge && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-blue-gradient text-white text-xs font-extrabold px-4 py-1.5 rounded-full shadow-blue-glow flex items-center gap-1.5 uppercase tracking-wider whitespace-nowrap">
              ...
              <div>
                ...
                <div className="mb-6 pt-2">
                  <h3 className="font-display font-bold text-xl sm:text-2xl text-white mb-2 flex items-center gap-2">
                    {stand.name}
                    {isPopular && <Star className="w-5 h-5 text-gold fill-gold shrink-0" />}
                  </h3>
                ...
                {/* Bloc Prix */}
                <div className="mb-8 p-4 rounded-xl bg-navy-main/70 border border-white/5 flex items-baseline gap-2">
                  <span className="font-data font-black text-3xl sm:text-4xl text-white tracking-tight tabular-nums">
                    {stand.priceFormatted}
                  </span>
                  <span className="font-body text-sm font-semibold text-brand-light uppercase">
                    FCFA
                  </span>
                </div>
                ...
                <div className="space-y-3.5 mb-8">
                  ...
                  {stand.features.map((feature, idx) => (
                    <div key={idx} className="flex items-start gap-3 text-sm text-slate-200">
                      <div className="w-5 h-5 rounded-full bg-brand-light/10 text-brand-light flex items-center justify-center shrink-0 mt-0.5 border border-brand-light/20">
              ...
              <button
                ...
                className={`w-full py-3.5 px-6 rounded-full font-bold text-sm flex items-center justify-center gap-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-brand-light ${
                  isPopular
